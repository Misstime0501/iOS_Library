...
@class HAObject, HASchema, HAMigration, HANotificationToken;

/**
 An HARealm instance (also referred to as "a realm") represents a Realm
 database.

 Realms can either be stored on disk (see +[HARealm realmWithPath:]) or in
 memory (see +[HARealm inMemoryRealmWithIdentifier:]).

 HARealm instances are cached internally, and constructing equivalent HARealm
 ...
 HARealm object. If you specifically want to ensure a HARealm object is
 ...
 @warning HARealm instances are not thread safe and can not be shared across
...
/**
 ...
 The default Realm is used by the `HAObject` class methods
 which do not take a `HARealm` parameter, but is otherwise not special. The
 ...
 @return The default `HARealm` instance for the current thread.
 */
+ (instancetype)defaultRealm;

/**
 Obtains an `HARealm` instance persisted at a specific file path.

 @param path Path to the file you want the data saved in.

 @return An `HARealm` instance.
 */
+ (instancetype)realmWithPath:(NSString *)path;

/**
 Obtains an `HARealm` instance with persistence to a specific file path with
 options.
 ...
 `HARealm` exists. This means that you cannot open a Realm as both read-only
...
/**
 Obtains an `HARealm` instance persisted to an encrypted file.
 ...
 @return An encrypted `HARealm` instance.
 */
+ (instancetype)realmWithPath:(NSString *)path
...
/**
 Obtains an `HARealm` instance for an un-persisted in-memory Realm. The identifier
 ...
 reference to the `HARealm` object returned from this for as long as you want
 the data to last. Realm's internal cache of `HARealm`s will not keep the
 in-memory Realm alive across cycles of the run loop, so without a strong
 reference to the `HARealm` a new Realm will be created each time. Note that
 `HAObject`s, `HAArray`s, and `HAResults` that refer to objects persisted in a Realm have a
 strong reference to the relevant `HARealm`, as do `HANotifcationToken`s.
...
/**
 The HASchema used by this HARealm.
 */
@property (nonatomic, readonly) HASchema *schema;
...
typedef void(^HANotificationBlock)(NSString *notification, HARealm *realm);
...
/**
 Add a notification handler for changes in this HARealm.
 ...
 - `NSString` \***notification**:    The name of the incoming notification. See
                                     HARealmNotification for information on what
                                     notifications are sent.
 - `HARealm` \***realm**:           The realm for which this notification occurred

 @param block   A block which is called to process HARealm notifications.

 @return A token object which can later be passed to `-removeNotification:`
         to remove this notification.
 */
- (HANotificationToken *)addNotificationBlock:(HANotificationBlock)block;
 ...
 */
- (void)removeNotification:(HANotificationToken *)notificationToken;
...
/**
 Begins a write transaction in an `HARealm`.

 Only one write transaction can be open at a time. Write transactions cannot be
 nested, and trying to begin a write transaction on a `HARealm` which is
 already in a write transaction with throw an exception. Calls to
 `beginWriteTransaction` from `HARealm` instances in other threads will block
 until the current write transaction completes.

 Before beginning the write transaction, `beginWriteTransaction` updates the
 `HARealm` to the latest Realm version, as if refresh was called, and
 generates notifications if applicable. This has no effect if the `HARealm`
 was already up to date.

 It is rarely a good idea to have write transactions span multiple cycles of
 the run loop, but if you do wish to do so you will need to ensure that the
 `HARealm` in the write transaction is kept alive until the write transaction
 is committed.
 */
...
/**
 ...
 After this is called the `HARealm` reverts back to being read-only.
...
/**
 ...
 accessor objects. Any `HAObject`s which were added to the Realm will be
...
/**
 Update an `HARealm` and outstanding objects to point to the most recent data for this `HARealm`.
...
/**
 ...
 Disabling this on an `HARealm` without any strong references to it will not
 have any effect, and it will switch back to YES the next time the `HARealm`
 object is created. This is normally irrelevant as it means that there is
 nothing to refresh (as persisted `HAObject`s, `HAArray`s, and `HAResults` have strong
 references to the containing `HARealm`), but it means that setting
 `HARealm.defaultRealm.autorefresh = NO` in
...
/**
 Write a compacted copy of the HARealm to the given path.
...
/**
 Write an encrypted and compacted copy of the HARealm to the given path.
...
/**
 Invalidate all HAObjects and HAResults read from this Realm.

 An HARealm holds a read lock on the version of the data accessed by it, so
 that changes made to the Realm on different threads do not modify or delete the
 data seen by this HARealm. Calling this method releases the read lock,
 allowing the space used on disk to be reused by later write transactions rather
 than growing the file. This method should be called before performing long
 blocking operations on a background thread on which you previously read data
 from the Realm which you no longer need.

 All `HAObject`, `HAResults` and `HAArray` instances obtained from this
 `HARealm` on the current thread are invalidated, and can not longer be used.
 The `HARealm` itself remains valid, and a new read transaction is implicitly
...
/**
 ...
 on `HARealm` and on subclasses of `HAObject`. When added, all linked (child)
 ...
 `-[HAObject createInRealm:withObject]` to insert a copy of a persisted object
...
/**
 ...
 @param array   An enumerable object such as NSArray or HAResults which contains objects to be added to
                this Realm.
...
/**
 ...
 primary key. If no objects exist in the HARealm instance with the same primary key value, the object is
 ...
 Realm. Use `-[HAObject createOrUpdateInRealm:withObject:]` to copy values to
 a different Realm.

 @param object  Object to be added or updated.
 */
- (void)addOrUpdateObject:(HAObject *)object;
...
/**
 ...
 @param array  `NSArray`, `HAArray`, or `HAResults` of `HAObject`s (or subclasses) to be added to this Realm.
...
/**
 Delete an `NSArray`, `HAArray`, or `HAResults` of objects from this Realm.

 @param array  `HAArray`, `NSArray`, or `HAResults` of `HAObject`s to be deleted.
 */
...
/**
 ...
 @param migration   `HAMigration` object used to perform the migration. The
                    migration object allows you to enumerate and alter any
                    existing objects which require migration.

 @param oldSchemaVersion    The schema version of the `HARealm` being migrated.

 @return    Schema version number for the `HARealm` after completing the
            migration. Must be greater than `oldSchemaVersion`.
 */
typedef void (^HAMigrationBlock)(HAMigration *migration, NSUInteger oldSchemaVersion);
...
/**
 ...
 Before you can open an existing `HARealm` which has a different on-disk schema
 ...
 You should call this method before accessing any `HARealm` instances which
 ...
 @see               HAMigration
 */
+ (void)setDefaultRealmSchemaVersion:(NSUInteger)version withMigrationBlock:(HAMigrationBlock)block;
...
/**
 ...
 @see               HAMigration
 */
+ (void)setSchemaVersion:(NSUInteger)version forRealmAtPath:(NSString *)realmPath withMigrationBlock:(HAMigrationBlock)block;
...
/**
 ...
 @return            The version of the Realm at `realmPath` or HANotVersioned if the version cannot be read.
 */
...
// This system can be extended with the HAKeyValueStore interface to create nested
// namespaces as needed.

// Retrieve a persisted object with an NSString.
//
// @usage HAObject *object = HARealm.defaultRealm[@"name"];
// @param key The NSString used to identify an object
//
// @return    HAObject or nil if no object is stored for the given key.
//
...
//
// @usage HARealm.defaultRealm[@"name"] = object;
// @param obj     The object to be stored.
// @param key     The key that identifies the object to be used for future lookups.
//
//-(void)setObject:(HAObject *)obj forKeyedSubscript:(id <NSCopying>)key;
...
//
@interface HANotificationToken : NSObject
@end
